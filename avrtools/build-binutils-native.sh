...
test -f config.status || {
	../configure --disable-nls --disable-werror --enable-lto --prefix=$NATIVEPREFIX --with-mpc=$NATIVEPREFIX --with-mpfr=$NATIVEPREFIX --with-gmp=$NATIVEPREFIX >$LOGS/binutils-config-native.log 2>&1 || \
