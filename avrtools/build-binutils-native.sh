...
cd build/native/binutils-${BINUTILS_VERSION} || \
...
test -f config.status || {
	../configure --disable-nls --enable-shared --disable-werror --prefix=$NATIVEPREFIX --with-mpc=$NATIVEPREFIX --with-mpfr=$NATIVEPREFIX --with-gmp=$NATIVEPREFIX >$LOGS/binutils-config-native.log 2>&1 || \
